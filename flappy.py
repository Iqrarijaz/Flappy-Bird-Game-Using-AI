...
# CLASS BIRD
class Bird:
    ROTATION_VEL = 20  # how much rot in each frame
    MAX_ROTATION = 25  # how much the bird in going to tilt
    ANIMATION_TIME = 5  # flapping speed
    IMGS = BIRD_IMGS
    ...
    def __init__(self, x, y):  # constructor for class bird
        ...
        self.tilt = 0
        self.vel = 0
        self.img_number = 0  # which image is currently animated
    ...
    def move(self):
        self.frame_count += 1  # update the frames when the bird moves keep track of last jump
        ...
        if d >= 16:  # if the bird is falling, and it falls more than 16 pixels then stop falling and straight moving
            d = 16
        self.y = self.y + d
        if d < 0 or self.y < self.height + 50:  # (d<0) this is the case when bird is going upward
...
class Pipe:
    GAP = 200  # space in btw our pipes
    VEL = 5    # move speed of pipe
    ...
    def move(self):
        self.x -= self.VEL  # moving the pipe to left
...
def main(genomes, config):
    ...
    for _, g in genomes:  # looping through object
        net = neat.nn.FeedForwardNetwork.create(g, config)  # method to create nn
    ...
    while run:
        ...
        # this part is done to check in the case when 2 pipes appear on the screen that which is the pipe we are
        # evaluating on
        if len(birds) > 0:
            # changing the pipe index if bird passes it
        ...
        for pipe in pipes:
            for x, bird in enumerate(birds):
                # checking if the bird has hit the ground or if the pipe and bird collide or if the bird has touched
                # te top boundary
                ...
                if not pipe.passed and bird.x > pipe.x:  # if the passed is not set to true and bird has passed the
                    # pipe then set it to true
                    pipe.passed = True
                    for g in ge:
...
if __name__ == '__main__':
    ...
    config_path = os.path.join(local_dir, 'file.txt')
    run(config_path)
